public class SteeringWheel extends CarComponent{
  ...
  public SteeringWheel(String nameOfComponent, boolean isWorking, String type, double diameterInInches){
  ...
  public String steer(){
    if (getIsWorking()){
      if (((int)(Math.random() * 100 + 1) < (diameterInInches * 0.25))){
  ...
  public String turn(){
    if (getIsWorking()){
      if (((int)(Math.random() * 100 + 1) < (diameterInInches * 0.5))){
      return damage() + "The steering wheel no longer steers.";
    }
    else {
      return "The steering wheel steers.";
